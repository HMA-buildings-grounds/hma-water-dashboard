...
if raw_data:
    for sheet_name, rows in raw_data.items():
        ...
        try:
            ...
            for _, row in df.iterrows():
                d_val, t_val, m_val = str(row.iloc[idx_date]).strip(), str(row.iloc[idx_time]).strip(), str(row.iloc[idx_meter]).strip()
                ...
                if pd.notnull(ts):
                    readings.append({'Timestamp': ts, 'DateOnly': ts.date(), 'IsMorning': '8:00' in t_val, 'Reading': m_num})
        except: continue
    ...
    if readings:
        ...
        for d, g in df_readings.groupby('DateOnly'):
            dt_usage, ov_usage = g[~g['IsMorning']]['Usage'].sum(), g[g['IsMorning']]['Usage'].sum()
...
with v_left:
    ...
    if not master.empty:
        fig = go.Figure()
        if "Usage" in view:
            fig.add_trace(go.Scatter(x=master['Date'], y=master['Daytime'], name='Daytime', fill='tozeroy'))
            fig.add_trace(go.Scatter(x=master['Date'], y=master['Overnight'], name='Overnight', fill='tozeroy'))
        elif "LPCD" in view:
...
with v_right:
    ...
    st.plotly_chart(go.Figure(go.Indicator(mode="gauge+number", value=eff, gauge={'axis':{'range':[0,100]}, 'steps':[{'range':[0,50], 'color':"#E74C3C"}, {'range':[50,85], 'color':"#F39C12"}, {'range':[85,100], 'color':"#1ABB9C"}]})), use_container_width=True)
